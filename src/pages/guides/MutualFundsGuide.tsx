...
export default function MutualFundsGuide() {
  ...
  const sortedFunds = [...selectedFunds].sort((a, b) => {
    ...
    // If mode is 'best':
    //   If better is 'low', we want ascending (low first)
    //   If better is 'high', we want descending (high first)
    // If mode is 'worst':
    //   If better is 'low', we want descending (high first)
    //   If better is 'high', we want ascending (low first)
                  ...
                  {selectedFundIds.length > 1 && (
                    <div className="hidden sm:flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-1.5">
                      ...
                      {sortConfig && (
                        ...
                        >
                          {sortConfig.mode === 'best' ? 'Best First' : 'Worst First'}
                        </button>
                      )}
                    </div>
                  )}
                                  ...
                                  >
                                    {sortConfig?.key === row.key ? (
                                      <span className="text-[9px] font-black uppercase tracking-tighter">
                                        {sortConfig.mode === 'best' ? 'Best' : 'Worst'}
                                      </span>
                                    ) : (
                              ...
                              {sortedFunds.map(fund => {
                                ...
                                return (
                                  <td key={fund.id} className={cn(
                                    "py-5 px-6 relative",
                                    getIsBestInRow(fund.id, row.key) && "bg-emerald-50/20"
                                  )}>
                                    {getIsBestInRow(fund.id, row.key) && (
                                      <div className="absolute top-0 right-0 p-1">
                                        <div className="bg-emerald-500 text-white p-0.5 rounded-full shadow-sm">
